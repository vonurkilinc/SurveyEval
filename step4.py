...
class CQSConfig:
    ...
    exag_tau: float = 0.25
    # New: reference-free exaggeration saturation scale (larger -> slower saturation)
    free_exag_tau: float = 0.25
...
def compute_cqs_for_pairs(
    ...
    # Global (reference-derived) mean/std in our descriptor space for "reference-free" exaggeration.
    # This represents a dataset-level "average face" target (not person-specific).
    ...
    if cfg.use_distinctiveness:
        ...
        if len(G_list) > 0:
            ...
            global_mean = internal_mean
            global_std = internal_std
        else:
            # Cannot compute distinctiveness without any valid references.
            stats_mean, stats_std = None, None
            global_mean, global_std = None, None
    else:
        # Even if distinctiveness isn't requested, we still want reference-free exaggeration.
        refs_df = (
            pairs_ok[["person_id", "reference_path"]]
            .drop_duplicates()
            .reset_index(drop=True)
        ...
        if len(G_list) > 0:
            G_stack = np.stack(G_list, axis=0).astype(np.float32)
            global_mean = np.mean(G_stack, axis=0)
            global_std = np.std(G_stack, axis=0)
    ...
    for _, row in pairs_ok.iterrows():
        ...
        # Reference-free exaggeration: deviation from a dataset-level "average face"
        # (this does NOT care whether exaggeration matches the person-specific distinctive features).
        mag_free = np.nan
        Sexag_free = 0.0
        if global_mean is not None and np.shape(global_mean) == np.shape(G_c):
            dg_free = np.abs(G_c - global_mean).astype(np.float32)
            mag_free = float(np.linalg.norm(dg_free))
            Sexag_free = float(1.0 - math.exp(-mag_free / (cfg.free_exag_tau + cfg.geo.eps)))
            Sexag_free = float(max(0.0, min(1.0, Sexag_free)))
        ...
        rows_out.append(
            dict(
                ...
                mag=float(mag),
                mag_free=float(mag_free) if not np.isnan(mag_free) else np.nan,
                Sconc=float(Sconc),
                Sdist=float(Sdist),
                Ssal=float(Ssal),
                Sconf=float(Sconf),
    ...
    if not df_out.empty:
        summary = (
            ...
            .agg(
                ...
                sexag_free_mean=("Sexag_free", "mean"),
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--free_exag_tau", type=float, default=0.25, help="Reference-free exaggeration saturation scale (new)")
...
def main() -> None:
    ...
    cfg = CQSConfig(
        ...
        free_exag_tau=float(args.free_exag_tau),
